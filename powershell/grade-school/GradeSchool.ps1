...
class Roster {
    ...
    [bool] AddStudent([int] $gp, [string] $np) {
        $AlreadyEnrolled = ($np -in $this.Students.Name)
        if (-not $AlreadyEnrolled) {
            $this.Students += , [Student]::new($gp, $np)
            Return $true
        }
        Return $false
    }

    [Student[]] GetRosterOfGrade([int]$gp) {
        [Student[]] $Result = $this.Students |
            Where-Object { $_.Grade -eq $gp } |
            Sort-Object -Property Name
        Return $Result
    }

    [Student[]] GetRoster([int]$gp) {
        Return $this.GetRosterOfGrade($gp)
    }

    [Student[]] GetRoster() {
        $Grades = $this.Students.Grade |
            Sort-Object -Unique
        [Student[]] $Result = @()
        foreach ($grade in $Grades) {
            [Student[]] $StudentsInGrade =
                $this.GetRosterOfGrade($grade)
            $Result += $StudentsInGrade
        }
        Return $Result
    }
}
