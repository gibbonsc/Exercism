...
<#
    # Extra challenge: remove the block comment delimiters to include this test
    It "nested arrays with mixed data types and null values" {
        $got  = Invoke-FlattenArray -Array ('ace')[0], $true, @(
                $null,@($false, (1/2)),@(@([long]::MaxValue)),@($null),[char]122
            )
        $want = "a", $true, $false, 0.5, 9223372036854775807, [char]'z'
