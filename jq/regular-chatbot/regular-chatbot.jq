...
def check_phone_number:
  capture("
    (?<country_code> (\\(\\+\\d+\\))? )  # reluctant quantifier
    \\s*                                 # possible whitespace before number
    (?<phone_number> \\d+-\\d+-\\d+ )    # such as 202-555-1212
  "; "x")
  |
  if .country_code != "" then
      "Thanks! Your phone number is OK."
  else
      "Oops, it seems like I can't reach out to \(.phone_number)."
  end
;

# Give a certain response from the user, help the Chatbot get
