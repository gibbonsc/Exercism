...
(
    ...
    | select(.item=="sugar")  # filter to just sugar
    | .amount.quantity
),
...
(
    .ingredients + ."optional ingredients"  # combine all ingredients
    | [
        .[]  # separate each ingredient
        | select(.substitute != null)  # discard the ones without substitutes
    ]  # collect filtered elements back into a new array
    | map( {(.item):(.substitute)} )  # convert each into a customized object
    | add  # merge array of custom objects into just one custom object
)
