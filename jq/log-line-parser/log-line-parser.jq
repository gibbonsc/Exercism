...
def message:
    # use slicer syntax [:] to slice out the message
    # use the index function to find the boundary between level and message
    # argument to index has length 3, so add 3 to index before slicing
    . [ index("]: ") + 3 :]
    | trim
;
